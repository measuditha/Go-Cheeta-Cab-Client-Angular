...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  userType: string;
  locations: string[] = ['--None--', 'Galle', 'Kandy' , 'Nugegoda', 'Gampaha', 'Kurunegala', 'Jaffna'];
  ...
  constructor(private carService: CarService, private userService: UserService, private dialog: MatDialog, private router: Router) {
    ...
    if (this.userService.isLoggedIn) {
      ...
      this.carService.getAllCars().subscribe(data => {
        const cars: Car[] = data;
        console.log(data);
        this.displayedColumns = ['brand', 'model', 'registrationNo', 'price', 'availability', 'locationId', 'driverId', 'action'];
  ...
  booknow(row: Car) {
    ...
    bookingData.driverId = row.driverId;
  ...
  onAddEdit(data) {
      if (data) {
        ...
        car.driverId = this.userService.userName;
